...
func TestFindExistingHost(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			hostName:       "name",
			provisioningID: "uuid",
			ironic:         testserver.NewIronic(t).NoNode("myns.name").NoNode("uuid"),
		},
		{
			name:           "by-name",
			hostName:       "name",
		...
		},
		{
			...
			hostName:       "name",
			provisioningID: "uuid",
			ironic: testserver.NewIronic(t).NoNode("myns.name").
				Node(nodes.Node{
					Name: "myns.different-name",
					UUID: "uuid",
				}),
			nodeName: "myns.different-name",
		},
	}
