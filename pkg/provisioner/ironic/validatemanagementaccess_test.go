...
func TestValidateManagementAccessNoMAC(t *testing.T) {
	...
	ironic := testserver.NewIronic(t).Ready().NoNode(host.Namespace + "." + host.Name)
	ironic.Start()
	defer ironic.Stop()
...
func TestValidateManagementAccessCreateNodeNoImage(t *testing.T) {
	...
	ironic := testserver.NewIronic(t).Ready().CreateNodes(createCallback).NoNode(host.Namespace + "." + host.Name)
	ironic.Start()
	defer ironic.Stop()
...
func TestValidateManagementAccessLinkExistingIronicNodeByMAC(t *testing.T) {
	...
	ironic.AddDefaultResponse("/v1/nodes/myns.myhost", "GET", http.StatusNotFound, "")
